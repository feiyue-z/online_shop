...
import com.github.klefstad_teaching.cs122b.gateway.model.AuthenticateResponse;
...
public class AuthFilter implements GatewayFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return authenticate(auth.get())
            .flatMap(
                result -> result == IDMResults.ACCESS_TOKEN_IS_VALID ? chain.filter(exchange) : setToFail(exchange)
            );
    }
    ...
    private Mono<Result> authenticate(String accessToken) {
        ...
        return webClient.post()
            .uri(config.getIdmAuthenticate().toString())
            .bodyValue(request)
            .retrieve()
